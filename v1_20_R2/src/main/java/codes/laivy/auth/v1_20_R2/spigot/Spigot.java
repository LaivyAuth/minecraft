package codes.laivy.auth.v1_20_R2.spigot;
...
import codes.laivy.auth.v1_20_R2.reflections.PlayerReflections;
    ...
    protected @UnknownNullability Object read(@NotNull ChannelHandlerContext context, @NotNull Object message) {
        ...
        if (message instanceof @NotNull PacketHandshakingInSetProtocol packet) {
            ...
            try {
                ...
                @Nullable LoginListener listener = (LoginListener) manager.m(); // Login Listener

                @NotNull SocketAddress remoteAddress = manager.f(); // Remote Address
        ...
        if (message instanceof @NotNull PacketLoginOutEncryptionBegin begin) {
            ...
            if (getApi().getConfiguration().getPremiumAuthentication().isEnabled()) {
                ...
                if (connection.getType() == null) {
                    ...
                    @Nullable LoginListener listener = (LoginListener) getNetworkManager(channel).m();
